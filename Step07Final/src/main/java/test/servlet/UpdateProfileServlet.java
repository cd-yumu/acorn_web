...
public class UpdateProfileServlet extends HttpServlet {
	// 파일 처리는 do post 메소드를 사용해 처리한다. (service 메소드를 오버라이드 받을 필요 없음)
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//이미 저장된 프로필 이미지가 있는지 여부 (위에 파일 관련 어노테이션 덕분에 원래는 안되지만 추출된다)
		...
		// 기존 파일이 있는 상태에서 수정하지 않았다면 filePart 는 null 이 아니지만 size 는 0이다.
