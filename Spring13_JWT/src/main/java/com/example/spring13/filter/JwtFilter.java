	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if(jwtToken != null) {
			...
			if(util.validateToken(jwtToken)) {
				// Spring Security 에 1 회성 로그인 처리를 해주고 요청의 흐름을 이어간다.
				filterChain.doFilter(request, response);
				return ;
			}	// 이번 예제에서는 Security 를 실제로 사용하지 않기 때문에 코드 양이 적다.(Security 추가하면 더 많아짐)
		}
		
		// 에러를 응답한다.
		response.sendError(HttpServletResponse.SC_FORBIDDEN, "Auth Fail!");
		// Security 를 실제로 추가하지 않아서 위 코드가 필요
		// Security 를 실제로 추가하면 알아서 처리된다.
	}
}
