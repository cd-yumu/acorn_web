...
public class CustomUserDetailsService implements UserDetailsService {
	...
	// 로그인 처리에 사용될 서비스
	@Override
	public UserDetails loadUserByUsername(String userId) throws UsernameNotFoundException {	// /user/login/요청이 오면 Spring Security 가 직접 호출한다.
		// DB 에서 Dao 를 애용해 username 에 해당하는 사용자 정보(UserDto) 를 얻어와야 한다.
		UserDto dto = dao.getDataById(userId);
