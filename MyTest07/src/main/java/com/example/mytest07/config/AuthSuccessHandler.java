...
public class AuthSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler {
    ...
    // 입력한 id 와 pwd 가 일치해서 로그인이 성공하면 이 메소드가 호출된다
 	@Override
 	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
         ...
         session.setMaxInactiveInterval(60*60);//초단위로 설정
         
         //Authentication 객체의 메소드를 이용해서 지금 로그인된 사용자에 대한 자세한 정보를 얻어낼수 있다.
         String userId = authentication.getName();
         System.out.println("로그인된 사용자(userId):"+userId);
 		
         
         // 로그인 된 userName 을 session 영역에 담아놓으면 jsp or thymeleaf 에서 편리하게 사용할 수 있다.
         // jsp 			=> ${sessionScope.userName} or ${userName}  (el 사용)
         ...
         if(cashed==null) {
         	//5. 로그인 성공 후 이동될 페이지 (POST 방식 요청임에 주의!!!)
