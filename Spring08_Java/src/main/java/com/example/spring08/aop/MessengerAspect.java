...
public class MessengerAspect {
	...
	public Object checkReturn(ProceedingJoinPoint joinPoint) throws Throwable{
		// aspect 가 적용된 메소드를 호출하고 해당 메소드가 리턴하는 값을 변수에 담기
	...
	// .. 은 매개 변수의 모양을 상관하지 않겠다. 는 의미
	// 아무것도 없어도 되고, 하나만 있어도 되고 여러개 있어도 되고 타입도 상관 안 함
	// (매개 변수 입장에서는 와일드 카드인 셈)
	@Around("execution(void send*(..))")
	public void checkGreeting(ProceedingJoinPoint joinPoint) throws Throwable {
		// joinPoint는 그 해당 지점을 가르킨다.
		// .getxxx 메소드로 어떤 정보를 얻어낼 수 있다.
		
		// 메소드에 전달된 인자들 목록 얻어내기
		Object[] args = joinPoint.getArgs();
		// 반복문 돌면서 매개 변수에 담긴 값들을 하나씩 조사해 볼 수 있다.
		for(Object tmp:args) {
			// 찾는 type 을 확인한다.
			if(tmp instanceof String) {
				...
				if(msg.contains("맹구")) {
					System.out.println("맹구는 금지된 단어 입니다.");
					return; // 메소드를 여기서 끝내기
				}
			}
		}
		
		// 이 메소드를 호출하는 시점에 실제로 aspect 가 적용된 메소드가 수행된다. 
		// (호출하지 않으면 수행 안 됨)
		joinPoint.proceed();
